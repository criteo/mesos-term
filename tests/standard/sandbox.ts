...
describe("Sandbox", function () {
  ...
  describe("file description is updated when file selected", () => {
    it("should update description", async function () {
      ...
      await Helpers.withChrome(async (driver) => {
        ...
        await Sandbox.checkDescription(
          driver,
          "stdout",
          "-rw-r--r--",
          "myuser",
  ...
  describe("open a file", () => {
    it("should open the stdout file", async function () {
      ...
      await Helpers.withChrome(async (driver) => {
        ...
        await driver.wait(
          Sandbox.untilFileReaderContains("MARATHON_APP_ID=/app1"),
          TIMEOUT_DRIVER
        );
      });
    });
  ...
  describe("tail", () => {
    it("should follow new lines coming in", async function () {
      ...
      await Helpers.withChrome(async (driver) => {
        await driver.get(
          `http://${username}:password@localhost:3000/task/${instanceID}/terminal?screenReaderMode=true`
        );
        await driver.wait(untilTermContains(/\$|#/), TIMEOUT_DRIVER);
        await sendKeysToTerminal(
          driver,
          "touch hello && sleep 5 && echo hello > hello\n"
        );
        ...
        await Sandbox.waitFileReaderToBeVisible(driver);
        await driver.wait(
          Sandbox.untilFileReaderContains("hello"),
          TIMEOUT_DRIVER
        );
      });
    });
