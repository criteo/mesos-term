  ...
  return new Condition(
    ...
    async function (driver) {
      const el = await driver.wait(
        until.elementLocated(By.className("xterm-accessibility-tree")),
        TIMEOUT_DRIVER
      );
...
export async function sendKeysToTerminal(driver: WebDriver, keys: string) {
  let el = await driver.wait(
    until.elementLocated(By.css(".xterm")),
    TIMEOUT_DRIVER
  );
  await el.click();
  el = await driver.wait(
    until.elementLocated(By.css(".xterm-helper-textarea")),
    TIMEOUT_DRIVER
  );
...
export async function testAnswerToCommand(
  driver: WebDriver,
  command: string,
  expected: string
) {
...
export async function checkInteractionsWithTerminal(
  ...
  await helpers.withChrome(async function (driver) {
    await driver.get(
      `http://${user}:password@localhost:3000/task/${instanceId}/terminal?screenReaderMode=true`
    );
...
async function enterAccessToken(driver: WebDriver, accessToken: string) {
  let el = await driver.wait(
    until.elementLocated(
      By.css("#access-request-dialog .token-field .MuiInput-input")
    ),
    TIMEOUT_DRIVER
  );
  el = await driver.wait(until.elementIsVisible(el), TIMEOUT_DRIVER);
  await el.sendKeys(accessToken);
  let buttonEl = await driver.wait(
    until.elementLocated(By.css("#access-request-dialog .ok-button")),
    TIMEOUT_DRIVER
  );
...
async function testReceiveErrorMessage(
  ...
  await helpers.withChrome(async function (driver) {
    ...
    await driver.wait(
      until.elementTextContains(el, expectedError),
      TIMEOUT_DRIVER
    );
  });
}
...
export function testCaseUnauthorizedAccessDialogDisplayed(
  user: string,
  appName: string
) {
  describe("unauthorized access dialog be displayed", () => {
    it("displayed", async function () {
      ...
      await helpers.withChrome(async function (driver) {
        ...
        const el = await driver.wait(
          until.elementLocated(By.id("access-request-dialog")),
          TIMEOUT_DRIVER
        );
...
export function testShouldNotSeeGrantAccessButton(
  user: string,
  appName: string
) {
  it("should not see the `Grant access` button", async function () {
    ...
    await helpers.withChrome(async function (driver) {
      ...
      let el = await driver.wait(
        until.elementLocated(By.css(".user-item")),
        TIMEOUT_DRIVER
      );
      await driver.wait(until.elementIsVisible(el), TIMEOUT_DRIVER);
...
export function testShouldSeeGrantAccessButton(user: string, appName: string) {
  it("should see the `Grant access` button", async function () {
    ...
    await helpers.withChrome(async function (driver) {
      await driver.get(
        `http://${user}:password@localhost:3000/task/${instanceId}/terminal`
      );
      const el = await driver.wait(
        until.elementLocated(By.css(".grant-permission-button")),
        TIMEOUT_DRIVER
      );
      await driver.wait(until.elementIsVisible(el), TIMEOUT_DRIVER);
    });
...
async function delegateAccessToken(
  ...
  const grantEl = await driver.wait(
    until.elementLocated(By.css(".grant-permission-button")),
    TIMEOUT_DRIVER
  );
  ...
  const dialogEl = await driver.wait(
    until.elementLocated(By.id("delegation-dialog")),
    TIMEOUT_DRIVER
  );
  await driver.wait(until.elementIsVisible(dialogEl), TIMEOUT_DRIVER);
  const userEl = await waitUntilElementIsVisible(
    driver,
    "#delegation-dialog .username-field .MuiInput-input",
    TIMEOUT_DRIVER
  );
  await userEl.sendKeys(delegatedUser);

  const select = await driver.wait(
    until.elementLocated(By.css("#duration-select")),
    TIMEOUT_DRIVER
  );
  await select.click();
  const option = await driver.wait(
    until.elementLocated(
      By.xpath('//*[@id="menu-"]/div[3]/ul/li[@data-value="' + duration + '"]')
    ),
    TIMEOUT_DRIVER
  );
  await option.click();

  const generateButtonEl = await waitUntilElementIsVisible(
    driver,
    "#delegation-dialog .generate-button",
    TIMEOUT_DRIVER
  );
  await generateButtonEl.click();

  const tokenEl = await waitUntilElementIsVisible(
    driver,
    "#delegation-dialog .token-field .MuiInput-input",
    TIMEOUT_DRIVER
  );
...
export function testShouldGrantAccessViaButtonAndToken(
  admin: string,
  delegatedUser: string,
  appName: string
) {
  it(`should allow ${admin} to delegate access to ${delegatedUser} via button and token in dialog`, async function () {
    ...
    await helpers.withChrome(async function (driver) {
      await driver.get(
        `http://${admin}:password@localhost:3000/task/${instanceId}/terminal?screenReaderMode=true`
      );
    ...
    await helpers.withChrome(async function (driver) {
      await driver.get(
        `http://${delegatedUser}:password@localhost:3000/task/${instanceId}/terminal?screenReaderMode=true`
      );
      await driver.wait(
        until.elementLocated(By.id("access-request-dialog")),
        TIMEOUT_DRIVER
      );
      const aTokenEl = await waitUntilElementIsVisible(
        driver,
        "#access-request-dialog .token-field .MuiInput-input",
        TIMEOUT_DRIVER
      );
      await aTokenEl.sendKeys(token);
      const okButtonEl = await waitUntilElementIsVisible(
        driver,
        "#access-request-dialog .ok-button",
        TIMEOUT_DRIVER
      );
...
export function testShouldGrantAccessViaButtonAndUrl(
  admin: string,
  delegatedUser: string,
  appName: string
) {
...
export function testShouldAbortAccessDelegation(
  admin: string,
  appName: string
) {
  it(`should face an error when user is not provided`, async function () {
    ...
    await helpers.withChrome(async function (driver) {
      ...
      const buttonClose = await driver.wait(
        until.elementLocated(By.css("#delegation-dialog .close-button"))
      );
