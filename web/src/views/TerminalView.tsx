...
export default function () {
  ...
  const [accessToken, setAccessToken] = useState<string | null>(
    qs.access_token ? (qs.access_token as string) : null
  );
  ...
  const retrieveConfig = useCallback(async () => {
    try {
      ...
      createErrorNotification(
        `Unable to retrieve configuration: ${err.message}`
      );
    }
  }, [createErrorNotification]);
    ...
    <div className={classes.root}>
      ...
      {taskID ? (
        <AccessRequestDialog
          ...
          onAccessRequest={setAccessToken}
        />
      ) : null}
      ...
      <div className={classes.statusBarContainer}>
        <div className={classes.statusBar}>
          <StatusBarItem
            value={context ? context.user : "_______"}
            icon={faUser}
            copy
            className={"user-item"}
          />
          <StatusBarItem
            value={context ? context.taskID : "____________________"}
            icon={faChevronRight}
            copy
          />
          <StatusBarItem
            value={context ? context.agent : "____________________"}
            icon={faServer}
            copy
          />
          {config && config.can_grant_access && status === Status.Loaded ? (
            <Button
              style={{ padding: theme.spacing(0.3) }}
              variant="outlined"
    ...
    >
      <div
        className={classnames(
          classes.root,
          props.className,
          props.copy ? classes.copyPointer : ""
        )}
        onClick={handleTooltipOpen}
      >
