...
import FileDescriptionBar, {
  Layout,
} from "../../components/FileDescriptionBar";
...
function buildURL(taskID: string, path: string, layout: Layout) {
  ...
  return `/task/${taskID}/sandbox?path=${encodeURIComponent(
    path
  )}&layout=${lay}`;
}
...
export default function () {
  ...
  const [layout, setLayout] = useState(
    qs.layout && qs.layout === "grid" ? Layout.Grid : Layout.List
  );

  const [files, setFiles] = useState(null as FileDescription[] | null);
  const history = useHistory();
  const [selectedFile, setSelectedFile] = useState(
    null as FileDescription | null
  );
  ...
  const restBreadCrumbItems = path
    .slice(1)
    .split("/")
    .filter((x) => x !== "")
    .map((x, i) => {
  ...
  return (
    <div className={classes.root}>
      ...
      {isFile ? (
        ...
        <div
          className={classes.explorerContainer}
          onClick={(e) => {
            setSelectedFile(null);
            e.preventDefault();
          }}
