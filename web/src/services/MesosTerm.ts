...
export async function postCreateTerminal(
  taskID: string,
...
export async function postResizeTerminal(
  token: string,
  rows: number,
  cols: number
) {
  const res = await Axios.post(
    `/api/terminals/resize?cols=${cols}&rows=${rows}&token=${token}`,
    null,
    {
      withCredentials: true,
    }
...
export async function postGenerateDelegationToken(
  request: GenerateDelegationRequest
) {
  const res = await Axios.post<GenerateDelegationResponse>(
    `/api/delegate`,
    request,
    {
      validateStatus: (code: number) => true,
...
export async function browseSandbox(taskID: string, path: string) {
  const res = await Axios.get<FileDescription[]>(
    `/api/sandbox/browse?taskID=${taskID}&path=${encodeURIComponent(path)}`,
    {
...
export async function readSandboxFile(
  ...
  const res = await Axios.get<FileData>(
    `/api/sandbox/read?taskID=${taskID}&path=${encodeURIComponent(
      path
...
export async function downloadSandboxFile(
  ...
  (window as any).location = `/api/sandbox/download?taskID=${taskID}&path=${encodeURIComponent(
    path
