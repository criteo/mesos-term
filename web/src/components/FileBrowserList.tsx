...
export default function (props: Props) {
  ...
  const directories = props.files
    ? props.files.filter((f) => f.mode.slice(0, 1) === "d")
    : [];
  ...
  const fileItems = dirsAndFiles.map((f, i) => {
    ...
    return (
      <FileItem
        ...
        selected={
          props.selectedFilePath !== null && f.path === props.selectedFilePath
        }
      />
    );
  });
...
function FileItem(props: FileItemProps) {
  ...
  return (
    <TableRow
      className={classnames(classes.root, "file-item")}
      hover
