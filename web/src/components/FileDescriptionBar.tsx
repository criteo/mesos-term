...
export default function (props: FileDescriptionBarProps) {
  ...
  const mtime = props.fd
    ? Moment(new Date(props.fd.mtime * 1000)).format(DATETIME_FORMAT)
    : "-";
  ...
  return (
    <Fragment>
      {!props.hideLayoutButtons ? (
        <ButtonGroup className={classes.layoutButtons}>
          <Button
            disabled={props.layout === Layout.Grid}
            onClick={handleLayoutGridClick}
          >
            <FontAwesomeIcon icon={faThLarge} />
          </Button>
          <Button
            disabled={props.layout === Layout.List}
      ...
      <span className={classnames(classes.item, "desc-name", "odd")}>
        name: {filename}
      </span>{" "}
      <span className={classnames(classes.item, "desc-size", "even")}>
        size: {size}kb
      </span>{" "}
      <span className={classnames(classes.item, "desc-mode", "odd")}>
        mode: {mode}
      </span>{" "}
      <span className={classnames(classes.item, "desc-mode", "even")}>
        date: {mtime}
      </span>{" "}
      <span className={classnames(classes.item, "desc-uid", "odd")}>
        uid: {uid}
      </span>{" "}
      <span className={classnames(classes.item, "desc-gid", "even")}>
        gid: {gid}
      </span>{" "}
