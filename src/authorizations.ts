...
export async function CheckUserAuthorizations(
  ...
  const admins_constraints_lower = admins_constraints.map((array) =>
    array.map((el) => el.toLowerCase())
  );
  ...
  if (admins_constraints.length == 0) {
    throw new UnauthorizedAccessError(
      "Only super admins can connect to this container"
    );
  }
...
export async function CheckDelegation(
  ...
  const payload = Jwt.verify(delegationToken, jwt_secret) as {
    task_id: string;
    delegate_to: string[];
  };
  ...
  const isUserDelegated =
    intersection(userAndGroups, payload.delegate_to).length > 0;
...
export async function CheckTaskAuthorization(
  ...
  await Promise.all([
    ...
    CheckRootContainer(task.user, userCN, userLdapGroups, superAdmins),
  ]);
}
