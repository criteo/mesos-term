...
function getOrElse<T extends string | number>(
  varName: string,
  defaultValue: T
): T {
  const v = process.env[varName];
...
const authorizations_enabled = process.env["MESOS_TERM_LDAP_URL"]
  ? true
  : false;

function getSuperAdmins() {
  const adminsStr = process.env["MESOS_TERM_SUPER_ADMINS"];
  return adminsStr ? adminsStr.split(",") : [];
}
...
export const env: EnvVars = {
  ...
  MESOS_STATE_CACHE_TIME: parseFloat(
    getOrExit("MESOS_TERM_MESOS_STATE_CACHE_TIME")
  ),
...
if (
  "MESOS_TERM_MESOS_AGENT_PRINCIPAL" in process.env &&
  "MESOS_TERM_MESOS_AGENT_PASSWORD" in process.env
) {
...
if (authorizations_enabled) {
  ...
  env["ENABLE_PER_APP_ADMINS"] =
    process.env["MESOS_TERM_ENABLE_PER_APP_ADMINS"] === "true";
  env["ENABLE_RIGHTS_DELEGATION"] =
    process.env["MESOS_TERM_ENABLE_RIGHTS_DELEGATION"] === "true";
