...
import {
  ...
  getMesosSlaveState,
  readSandboxFile,
  downloadSandboxFileAsStream,
  downloadSandboxDirectory,
  TaskInfo,
  MesosAgentNotFoundError,
  TaskNotFoundError,
  FileNotFoundError,
  findTaskInSlaveState,
  MesosTaskState,
} from "../mesos";
...
function cacheSandboxDescriptor(
  fetcher: (taskID: string) => Promise<SandboxDescriptor>
) {
  const cache: {
    [taskID: string]: {
      expireAt: Date;
...
const sandboxCache = cacheSandboxDescriptor(async (taskID) => {
  ...
  if (slaveTaskInfos.length > 1) {
    throw new Error(
      `Multiple task descriptors have been found for task ${taskID}`
    );
  }

  const status: TaskState =
    slaveTaskInfos[0].statuses.length > 0
...
export default function (app: Express.Application) {
    ...
    async function (
      ...
      try {
        if (req.user == undefined)
          console.log(
            `Anonymous connection to ${req.query.taskID}: ${req.query.path}`
          );
        else
          console.log(
            `Connection attempt from ${req.user.cn} for ${req.query.taskID}: ${req.query.path}`
          );
        if (env.AUTHORIZATIONS_ENABLED && !env.AUTHORIZE_ALL_SANDBOXES) {
          const sandbox = await sandboxCache(req.query.taskID);
          await CheckTaskAuthorization(
            req,
            sandbox.task,
        ...
        console.error(
          `Cannot authorize user ${req.user.cn} to access to sandbox of task ${req.query.taskID}`,
          err
        );
  ...
  app.get(
    ...
    async function (req: Express.Request, res: Express.Response) {
      try {
        const sandbox = await sandboxCache(req.query.taskID);
        const files = await browseSandbox(
          sandbox.agentURL,
          sandbox.workDir,
          sandbox.slaveID,
          sandbox.frameworkID,
        ...
        console.error(
          `Cannot browse files in ${req.query.path} from sandbox of task ${req.query.taskID}`,
          err
        );
        if (err instanceof MesosAgentNotFoundError) {
          res.status(400);
  ...
  app.get(
    ...
    async function (req: Express.Request, res: Express.Response) {
      try {
        const sandbox = await sandboxCache(req.query.taskID);
        const files = await readSandboxFile(
          sandbox.agentURL,
          sandbox.workDir,
          sandbox.slaveID,
          sandbox.frameworkID,
        ...
        if (
          !(
            sandbox.last_status === "TASK_RUNNING" ||
            sandbox.last_status === "TASK_STARTING"
          )
        ) {
          files.eof = true;
        }
        ...
        console.error(
          `Cannot read file ${req.query.path} from sandbox of task ${req.query.taskID}`,
          err
        );
        if (err instanceof MesosAgentNotFoundError) {
          res.status(400);
    ...
    async function (req: Express.Request, res: Express.Response) {
      try {
        ...
        if (req.query.directory === "true") {
          await downloadSandboxDirectory(
            sandbox.agentURL,
            sandbox.workDir,
            sandbox.slaveID,
            sandbox.frameworkID,
          ...
          await downloadSandboxFileAsStream(
            sandbox.agentURL,
            sandbox.workDir,
            sandbox.slaveID,
            sandbox.frameworkID,
        ...
        console.error(
          `Cannot download file(s) ${req.query.path} from sandbox of task ${req.query.taskID}`,
          err
        );
        if (err instanceof MesosAgentNotFoundError) {
          res.status(400);
